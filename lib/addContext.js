...
const ERRORS = {
    ...
    INVALID_CONTEXT: ctx => {
        const expected =
            'Expected a string or an object of shape { title: string, value: string } but saw:';
...
const addContext = function (...args) {
    ...
    if (args.length !== 2 || !isObject(args[0])) {
        lib.logDebug(ERRORS.INVALID_ARGS, 'error');
        return;
    }
    ...
    if (!_isValidContext(ctx)) {
        lib.logDebug(ERRORS.INVALID_CONTEXT(ctx), 'error');
        return;
    }
    ...
    if (!test) {
        lib.logDebug(ERRORS.INVALID_TEST, 'error');
        return;
    }
    ...
    if (!test.splunkContext) {
        test.splunkContext = ctx;
    } else if (Array.isArray(test.splunkContext)) {
        // Test has context and context is an array -> push new context
        test.splunkContext.push(ctx);
    } else {
        // Test has context and it is not an array -> make it an array, then push new context
        test.splunkContext = [test.splunkContext];
        test.splunkContext.push(ctx);
    }
};
